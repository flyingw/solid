...
impl<'a, D: DBAccess> DBRawIteratorWithThreadMode<'a, D> {
    ...
    //
    fn columns_impl(&self) -> &[u8] {
        unsafe {
            let mut len: size_t = 0;
            let len_ptr: *mut size_t = &mut len;
            let columns_ptr = ffi::rocksdb_iter_columns(self.inner.as_ptr(), len_ptr);
            slice::from_raw_parts(columns_ptr as *const c_uchar, len)
        }
    }
}
